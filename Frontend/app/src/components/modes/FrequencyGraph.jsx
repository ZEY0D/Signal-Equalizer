...
const FrequencyGraph = ({ initialData = generateDummySpectrum() }) => {
  ...
  const adjustedFrequencyData = useMemo(() => {
    ...
    return frequencyData.map((point) => {
      const totalGain = sliderList.reduce((acc, slider) => {
        ...
        if (point.frequency >= rangeStart && point.frequency <= rangeEnd) {
  ...
  const addSliderFromDraft = () => {
    const [start, end] = normalizeRange(draftRange.startHz, draftRange.endHz);
  ...
  const canAddDraft = draftRange.endHz - draftRange.startHz >= STEP_HZ;
    ...
    <section className="freq-root">
      ...
      <section className="range-planner">
        ...
        <p className="planner-text">
          Drag the handles to choose the frequency span, then click “Add slider”
          to pin it on the graph.
        </p>
      ...
      <div className="graph-card">
        ...
        >
          ...
          <path d={frequencyPath} className="frequency-path" />

          <g>
            {(scale === LINEAR_SCALE ? linearTicks : audiogramTicks).map(
              (tickHz) => {
                const clampedHz = Math.min(Math.max(tickHz, 0), safeMaxHz);
                if (scale === AUDIOGRAM_SCALE && tickHz < MIN_LOG_HZ)
                  return null;
        ...
        <div className="slider-list">
          ...
          {sliderList.length === 0 && (
            <p className="empty-state">
              No sliders yet — pick a range above and click “Add slider” to
              start sculpting.
            </p>
          )}
        </div>
